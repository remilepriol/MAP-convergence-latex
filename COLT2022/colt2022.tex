...
\coltauthor{%
 \Name{R\'emi {Le Priol}} \Email{remi.lp.17@gmail.com}\\
...
\textbf{ 2)} Distance between distributions, as studied in \emph{density estimation}.
...
\boxed{
\begin{aligned}
	\textit{Find an upper bound on the expected value of } \ \
	\KL(p_{\nat^*} || p_{\hat \nat_n^\text{MLE/MAP} }) \; .
\end{aligned}
}
\tag{$\star$}
...
Both MLE and MAP can be seen as stochastic algorithms minimizing $f$.
...
\textbf{ The Bregman Divergence} induced by $\logpart$ measures the discrepancy between two parameters $\nat$ and $\nat_0$,
\begin{align}
    \bregman (\nat ; \nat_0)
    & = \logpart(\nat) - \logpart(\nat_0)
    - \langle \nabla \logpart(\nat_0)  , \nat - \nat_0 \rangle,
...
\textbf{ A Conjugate Prior} for $p(X|\nat)$ is
\begin{align}
    p(\nat)
...
\textbf{ A Difficulty with the MLE.}
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=.8\textwidth]{thales/numerical_schema_n=3.pdf}
	\caption{
...
\textbf{ If $\conj$ is $L$-Lipschitz} (e.g. $\logpart$ is defined within the $\ell^2$-ball of radius $L$), then
\begin{align}
    \bregmanconj(\m^* ; \m)
    &\leq L \norm{\m^* - \m} + \norm{\nat} \norm{\m^* - \m} \\
    &\leq 2L \norm{\m^* - \m} \; ,
...
\begin{figure}[t]
	\centering
	\includegraphics[width=.4\textwidth]{gaussians/new_linear_n0=1.pdf}
	\caption{
...
\acks{ % Acknowledgments---Will not appear in anonymized version
...
\textbf{ Clarification:} In the main text in Section~\ref{ssec:local-quadratic}, we claimed that the Fenchel conjugate of a 1-dimensional function is also self-concordant. 
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.4\textwidth]{self_concordance/omega.eps}
	\includegraphics[width=0.4\textwidth]{self_concordance/omega_star.eps}
...
\textbf{ In our case,} where the objective function is the (negative) log-likelihood of an exponential family, we have
\alignn{
