...
Proofs are in \cref{app:asymptote}, and \citet[\S1.1]{ostrovskii2021finite} offers a great review.
...
} of $\conj$ \citep[Ch.4.1]{nesterov2003introductory}, we can quantify when this quadratic behavior kicks in. Proofs for this subsection are in \cref{app:self-concordant}.
\begin{proposition}
...
This means that on average, we need $5$ times more samples than the dimension to reach the quadratic regime where a formula like~\eqref{eq:MAP_quadratic} is valid.
...
A contrario, the MLE is unbiased w.r.t. the dual parameter, but the decomposition there is not clean.
...
Recent works extend these assumptions to the $\stgcvx$-strong convexity and $\smooth$-smoothness of $f$
\emph{relative} to $\logpart$,
\aligns{
	\stgcvx \cB_{A}(x, y)
	\leq
	\cB_f(x,y)
