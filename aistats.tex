...
\citet{hanzely2018fastest} assume the covariance between negative gradient and next iterate is upper bounded at every time step
\alignn{
\Cov(-g_n(\hat \nat_{n-1}) , \MAPt) \leq \lr_n C, \forall n \geq 1
\label{eq:assumption-hanzely}
}
for some constant $C$.
...
Under this assumption, they prove a $O(1/n)$ convergence rate with a $O(1/n)$ step-size \citep[Lem.4.8]{hanzely2018fastest} , which is precisely what we wish to obtain. 

In general however, this assumption hardly holds.
For the MAP, this translates into
\alignn{
\expect[T_n]{\lin{T_n - \mu^* , \MAPt}} \leq \lr_n C, \forall n \geq 1
}
...
\citep{dragomir2021fast} is similar to \cref{eq:hanzely-in-divergence},
where we take only the first part of it
and care only about gradients at the minimum, replacing
$\tilde g(\theta)$ and  $\nabla f(\theta)$ with $\tilde g(\theta_*)$ and $\nabla f(\theta_*) = 0$
\aligns{
	\expect[\tilde g]{
		\cB_{A^*}(\m - 2\eta \tilde g(\theta_*), \m)
	} \leq 2 \eta^2 C.
}
...
\fdk{How do we show that this fails?}
...
\\
Do they do something with that? It seems to be fixed step-size only.
}
\aligns{
	A(\theta_*) - \lin{x, \theta_*} - \expecti{ \min_\theta A(\theta) - \lin{x, \theta} }
}
