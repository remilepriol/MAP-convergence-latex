...
\begin{align}
	f(\nat) = \expect[X \sim \cD]{-\log p(X \cond \nat)} 
	 = \logpart(\nat) - \langle \E[T] , \nat \rangle
\end{align}
Up to a constant, it is also the KL  $f(\nat) = \KL(\cD || \nat) := \KL(\cD || p(\cdot \cond \nat))$.
...
\footnote{
Since for exponential families, $\bregman(\nat ; \nat^*) = KL(\nat^* || \nat)$, this proves on the way the Pythagorean theorem of information geometry $KL(\cD || \nat) = \KL(\cD || \nat^*) + \KL(\nat^* || \nat)$.
}
\begin{align}
	 f(\nat) - f(\nat^*)
	 %&= \logpart(\nat) - \logpart(\nat^*) - \langle \E[T] , \nat - \nat^* \rangle \\
...
\alignn{\label{eq:hanzely-in-divergence}
	\begin{aligned}
	&
	\mathbb{E}_{\tilde g}\bigg[
	\cB_{A^*}\paren{\m - \eta \tilde g(\theta), \m - \eta \nabla f(\theta)}
	\\[-1em]
	&\hphantom{\mathbb{E}_{\tilde g}[}
	+ \cB_{A^*}\paren{\m - \eta \nabla f(\theta), \m - \eta \tilde g(\theta)}
	\bigg]
	\leq
	\eta C.
	\end{aligned}
...
\fdk{Could work with decreasing step-sizes}

\fdk{How do we show that this fails?}

\subsection{Bounded variance at the minimum}
