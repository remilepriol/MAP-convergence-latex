...
{\bf Impact.}
While a solution to the fundamental problem~\eqref{problem} would be of independent interest -- e.g. assessing the importance of the prior in MAP with few samples-- we believe it would have repercussions to other fields. 
For instance, it could enable stochastic optimization to tackle a wider class of generalized linear models than fixed variance gaussians (linear regression) and Bernoulli (logistic regression) \citep{bach2013nonstronglyconvex}.
%Or it could help developing accelerated (stochastic?) algorithms for barrier objectives  overcoming the limitations of non-Euclidean geometry \citep{dragomir2021optimal}. \rlp{Fred I need your feeedback on that one.}

% outcomes
% quantifying the importance of the prior, and few sample behavior, for instance in Gaussians
% stochastic algorithms for barrier objectives
% such as a lot of GLM
% accelerated methods ?
