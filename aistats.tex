...
MLE and MAP only depend on $T(X)$, not $X$, so their performance only distributions of $T(X)$. 
...
This theorem applies to Gaussian variance since 
\TODO{explain how this corresponds to theorem 4.1}
...
\subsection{Proof for the MLE}
	...
	\begin{proof}
	For gamma distributions, since $T(X)=X$ the MLE is a scaled sum of gammas $\hat \mu_n = \inv{n} \sum_i X_i$.
...
Expressions for $n\alpha < 1$ are less elegant so we do not write them down here. 
\TODO{try simplification with wolfram alpha}.
