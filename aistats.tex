...
\begin{align}
	\logpart (\nat) &= - \half \log(-\nat)  + \half \log(\pi), \\
	\conj(\m) &= - \half \log(\m) + \half \log\frac{\pi}{2} - \half  \; .
\end{align}
...
In particular, there is no bias term when $\frac{\m_0}{\m^*} =1 + \inv{n_0} $, which happens when the prior is slightly larger than the ground truth, which correlates well with numerical observations (cf App.\ref{app:gaussian-variance}).
%For instance, when $n_0=1$, it encourages us to set $\m_0 = 2 \m^*$.
...
Equipped with the knowledge of these 2 examples, we are ready to discuss the general problem, and some ways to solve with direct expansions of the Bregman.
