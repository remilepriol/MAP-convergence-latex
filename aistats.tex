...
\subsection{Gaussian}
\label{ssec:gaussian}
The ubiquitous Gaussian $\cN(m,\sigma^2)$ actually offers a highly non-trivial example for Problem 1.
Its log-likelihood reads $p(x) = -\frac{(x-m)^2}{2 \sigma^2} - \half \log(2\pi\sigma^2)$.
...
$\cM$ and $\Theta$ are delimited in grey in \cref{fig:thales}.
Given these parameters, log-partition and entropy are
\alignn{
	\logpart(\nat) &= \frac{\nat_1^2}{-4\nat_2} - \half \log(-2\nat_2) \\
...
up to constants.
 ...
 Neither its log-partition nor its entropy are smooth, strongly-convex, or self-concordant.
