...
where $\logpart$ is a convex function and $\nat$ is a vector\textcolor{red}{DS: double check this}, called \textit{natural parameter}.
...
\textcolor{red}{First, i think its better to have a new paragraph here. Then you need to introduce the problematic (like: we need to estimate the parameter theta, and its easy with exponential families becase blabla) }Another interest of exponential families is their closed form maximum-likelihood estimate (MLE) obtained via moment matching:
\begin{align}
	\nabla \logpart(\hat \nat_n^\text{MLE}) = \frac{\sum_{i=1}^n T(x_i)}{n} .
	\label{eq:defMLE}
\end{align}
Given a conjugate prior, a similar formula holds for the maximum a posteriori (MAP). \textcolor{red}{DS: I know that for conciseness we do not write the MAP estimate here, but it's super central and I think its important to write the equation. WDYT?}

\textcolor{red}{This comes a bit from nowhere. Maybe phrase it like (its not sexy wirting here, just the idea of the structure) "The measure of closeness between theta and its estimate hat theta is an active area of research. We distinguish two way to do that: 1) distance in the parameter space and 2) distance between their ascociated distribution. In the first case, the analysis use norms blabla. In the second, we use KL"} How can we assess the quality of the MLE, or any other estimator?
...
\textcolor{red}{The sample complexity of the quality of the estimator is well known when we use norms between parameters, in opposition to the case where we measure the distance between distribution. Hence the natural question:} 
...
or close to quadratic, there are simple upper bounds. \textcolor{red}{Maybe say that "in this paper, we show that this problem is difficult and we give the reason why and some idea on how to solve it"}
...
This question has connections with optimization. \textcolor{red}{I would start with "Consider the problem $\min_{\theta\in \Theta} f(\theta)$ (write the equation in display). Setting $f=...$, we obtain that KL is the measure of suboptimality, i.e., (equation (3)). Moreover, we have that MAP solve a regularized version of f, (write equation) and MLE is the special case where (regularisation = 0)"}
...
%similarly to other works on stochastic approximation \citep{bach2013nonstronglyconvex}. %TODO check this
...
After formalizing the problem \textcolor{red}{restate the problem} along with its asymptotic properties, we make the following contributions. \textcolor{red}{Add ref to the main text in the contribution.}
\begin{itemize}
	\itemsep0em 
	\item We highlight sufficient conditions to characterize when a (local) quadratic approximation is valid. \textcolor{red}{Approx. of what?}
	\item We investigate a generalized bias-variance decomposition~\citep{pfau2013generalized}. \TODO{revealing what ? the ideal shape of a rate and a sub-problem} \textcolor{red}{Decomposition of what?}
	\item We provide a full solution \textcolor{red}{full solution of what?} in the case of a Gaussian with known mean but unknown variance $\cN(0,\sigma^2)$.
	\item We establish a connection \textcolor{red}{Connection between something (you need to precise what) and optimization} with optimization, and use this problem to highlight areas for progress in the stochastic optimization of barrier objectives.
\end{itemize}
...
\[ 
	\cX=\real^d,\quad \nu(dx) = \exp({\textstyle \half[-\|x\|^2]})dx,\quad T(x)=x.
\]
...
\section{Gaussian Variance Example}\label{sec:example}
