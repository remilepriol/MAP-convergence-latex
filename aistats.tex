...
\citep[p.177]{nesterov2003introductory} % example 4.1.1.4
...
\aligns{
	\bregmanconj( \mu_*; \mu_n) 
	&\leq \bregmanconj( \mu_*; \mu_n)  + \bregmanconj( \mu_n; \mu_*) \\ 
	=: \cS_{\conj} (\mu_*, \mu_n )  
...
\begin{theorem}[MAP Bound]
The expected symmetrized Bregman of the MAP of $\cN(0,\m^*)$ with prior hyper-parameters $(n_0,\m_0)$ is upper bounded as
 \begin{align}
	& \expect{\bregmanconj( \m^*; \hat \m_n^\mathrm{MAP})}
	\leq \begin{cases}
...
\subsection{Complementary Plots}

\rlp{Show that the optimal mu0 is larger than mu* for small n0.}
