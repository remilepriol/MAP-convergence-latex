...
After exhibiting various facets of the problem, we show the MAP can be interpreted as the result of Stochastic Mirror Descent (SMD), yet it falls out of scope for modern analysis.
...
As another reference point, let us consider the case $\logpart = \half \norm{\natp}_2^2$, which corresponds to finding the mean of a Gaussian with variance $1$ ($\cX=\real, \nu(dx) = \exp(\half[-x^2])dx, T(X)=X$).

If we make restrictive assumptions on the log-partition function, we can ensure our objective will behave at least as nicely as a quadratic.
\paragraph{If $\conj$ is $L$-Lipschitz} (e.g. $\logpart$ is defined within the $\ell^2$-ball of radius $L$, which kinda adresses \citet{bubeck2015entropic}), then
\begin{align}
    \bregmanconj(\mu^* ; \mu) 
    &\leq L \norm{\mu^* - \mu} + \norm{\natp} \norm{\mu^* - \mu} \\
    &\leq 2L \norm{\mu^* - \mu}
...
Instead of measuring strong-convexity and smoothness relative to the Euclidean norm (or any norm), 
...
\begin{itemize}
	...
	\item Attempt to adapt the ``bounded gradient at the minimum''
	\citep{dragomir2021fast}
	\item Adaptation of the $\expect{\min_x f(x) - \min_x f_i(x)} \leq \sigma^2$ hypothesis \citep{dorazio2021stochastic}.
\end{itemize}
...
\begin{enumerate}
	...
	\item Technical Background : be precise and add reference for the steep exponential family of Legendre type which guarantees that our sets are open, and have a solution in the middle. References: \begin{itemize}
