...
We believe solving this very fundamental problem may bring progress both to the statistics and optimization communities.
\end{abstract}
...
\begin{enumerate}
	...
	\end{enumerate}
	\item positioning between optimization and statistics. Start with stat. Convince the reader that this is an essential problem, with asymptotic rate. but finite is elusive. Inspired by recent work (lepriol , bach2013) we want to use optimization to give a rate, we try. but we reveal a hole in current analysis.  Finite rate would allow to see the importance of the prior, and also help us understand behavior for small numbers. 
\end{enumerate}
