...
\begin{important}
	{\bf Erratum:} contrary to what is written in the main text in the last paragraph of Section~\ref{ssec:gaussian} and~\ref{ssec:local-quadratic}, the log-partition of a full Gaussian \emph{is} self-concordant. See \cref{app:gaussian} for details. This does not change the narrative of the paper though.
\end{important}
...
\begin{important}
	{\bf Erratum:}  
	In the main text, we claimed by mistake in the last paragraph of Section~\ref{ssec:gaussian} and~\ref{ssec:local-quadratic} that the log-partition of a full Gaussian is not self-concordant. 
	There was an error in our derivations and we will now prove the opposite (using classical results).
	However, this does not change our main story, and in particular, the point that we made in the last paragraph of Section~\ref{ssec:local-quadratic} that there is a lack of results in the few sample regime remains valid.
...
% TODO We also give complementary illustrations on the Gaussian mirror map and MLE.
...
% TODO These functions and their domains are represented in Figure \TODO{}.
...
where the last equality holds by symmetry between the roles of $\logpart$ and $\conj$.
...
Taking the expectation over $\hat\mu_{n-1}$ instead yields
\alignn{
...
If assumption~\eqref{eq:hanzely} holds, then
\alignn{
\expect[1:n]{\bregmanconj(\tilde \m_n , \hat\m_n) }
