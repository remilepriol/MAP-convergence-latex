...
To address these problems, we start by investigating simple examples for which we can provide solutions to Problem 1, getting insights into what is achievable.
...
\label{ssec:gaussian}
Now that we have solved the case of $\cN(0,\sigma^2)$, let us consider the full Gaussian $\cN(m,\sigma^2)$ which actually offers a highly non-trivial example for Problem 1.
...
We first review the assumptions of relative smoothness, useful to deal with non-smooth functions, before investigating recent analyses of SMD with the MAP.
...
These relative conditions ensure the linear convergence of MD with mirror map $\nabla A$
\citep{birnbaum2011distributed, bauschke2017descent, lu2018relatively},
even when $f$ is not smooth and $\logpart$ is not strongly convex.
...
Several pieces of work have addressed this question, under various assumptions on the stochasticity \citep{hanzely2018fastest, dragomir2021fast, dorazio2021stochastic}, 
but they have so far been lacking concrete examples.
In the next section, we challenge their hypothesis thanks with the MAP.
