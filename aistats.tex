...
These results answer~\eqref{problem} only \emph{partially} for the following reasons.
...
Second, they do not apply to some standard exponential families like the full Gaussian example that we saw in \S\ref{ssec:gaussian} which is \emph{not} self-concordant. Third, Proposition~\ref{prop:selfConcordant} does not imply directly a bound on the \emph{expected} Bregman.\footnote{One way to see this is that while the entropy of the categorical distribution is self-concordant, and thus Proposition~\ref{prop:selfConcordant} applies to it, the expected KL of its MLE is still infinite as previously mentioned in \S\ref{sec:problem}.}
A full solution to~\eqref{problem} would address all of these caveats.
