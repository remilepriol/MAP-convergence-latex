...
\begin{figure}[ht]
	...
	% fdk: Text said that it admits an the grey upper bound because it is a barrier. Don't follow the logical implication.
	}
	\label{fig:phi}
\end{figure}
...
In higher dimensions, $X \sim \cN(0, \mu_*)$, $X\in\real^d$, $T(X) = XX^\top$, and the mean parameter $\mu_*$ is a symmetric positive definite covariance matrix of order $d$, living in dimension $p = \frac{d(d+1)}{2}$. 
Note that we switched the role of $d$ from $\dim(T(\cX))=d$ to $\dim(\cX) =d$ and $\dim(T(\cX))=p$, where $\dim(\cZ)$ is the dimension of the smallest affine subspace containing $\cZ$.
...
\begin{align}
 	\expect{\bregmanconj( \mu_*; \hat \mu_n^\text{MLE(d)})}
 	&\leq p \paren{ \inv{n-d-1} - \inv{2n}}  + \frac{d + 1}{4n(n - d - 1)}\\
 	&< p \frac{n + d +1}{2 n (n-d-1)}  + \frac{d(d + 1)/2}{2dn(n - d - 1)}\\
 	&= \frac{p}{2n}\frac{n-d-1}{n-d-1} + p\frac{2(d+1)}{2 n (n-d-1)} + \frac{p}{2dn (n - d - 1)}\\
...
\alignn{
	...
	Fortunately, there is a trick to express it as an integral itself , with exponential terms that fit well with the $\chi^2$-density,
	\fdk{I don't follow from there.} \rlp{do you have more detailed feedback ? I don't know how to improve it.}
\alignn{
...
\alignn{
	...
	%\fdk{Fubini's theorem would be a justification here, right? Are you trying to say ``assuming we can exchange the order of integration''?}\rlp{yes exactly.}
	...
	\fdk{Was this a comment from your notes that passed its way here or do you want to go with this?} \rlp{Yes I like this sentence.}
...
These functions and their domains are represented in Figure \TODO{}.
...
Adopting the notation of definition~\ref{def:self-concordance}, let us set $w=(\nat_1 \ 0)^\top$ and $v=(\alpha \ \ -1)^\top$. 
For varying $\nat_1, \alpha$, this covers the whole range of possible straight lines in the 2D plane, except lines of constant $\nat_2$ (horizontal).
But on lines with $\nat_2=\cst$,   the log-partition function $A$ is simply a quadratic, so it is self-concordant.
Then, up to constants,
\alignn{
g(t) := 2A(w+tv) 
= \frac{(\nat_1 + t \alpha)^2}{2 t} - \log(t) + \cst
= \frac{a}{t}  + b t - \log(t) + \cst
}
where $a=\frac{\nat_1^2}{2} \geq 0$, and $b = \frac{\alpha^2}{2}$.
$g$ is well defined for $t>0$.
The logarithm is self-concordant for the same reason that the entropy is, but the inverse function $\inv{t}$ is not \citep[example 4.1.1.5]{nesterov2003introductory}.
Linear combinations with positive coefficients  preserves self-concordance \citep[theorem 4.1.1]{nesterov2003introductory} -- e.g. self-concordance is a cone, so it seems unlikely that this sum of a self-concordant function and a not self-concordant function would be self-concordant. 
So let us prove it.
The derivatives of $g$ are
\alignn{
g'(t) &= -\frac{a}{t^2} - \frac{1}{t} + b \; ,\\
g''(t) &= \frac{2a}{t^3} + \inv{t^2} = \frac{1}{t^2}\paren{\frac{2a}{t} + 1} \; , \\
g'''(t) &= -\frac{6a}{t^4} - \frac{2}{t^3} = \frac{-2}{t^3}\paren{\frac{3a}{t} + 1} \;.
}
Then the definition of self-concordance reads
\alignn{
\abs{g'''(t)} &\leq 2 g''(t)^{\frac{3}{2}} \\
\iff \frac{3a}{t} + 1 &\leq \paren{\frac{2a}{t} + 1}^{\frac{3}{2}} \; .
}
Writing $y = \frac{a}{t} \geq 0$, and taking the square of this expression yields
\alignn{
 (3y + 1)^2 &\leq (2y + 1)^3 \\
 \iff 9 y ^2 + 6y + 1 &\leq 8 y^3  + 12 y^2 + 6y +1 \\
\iff 0 &\leq y^2(8y + 3)
}
which is true since $y\geq 0$.
The log-partition is self concordant.
...
\alignn{\abs{g'''(t)} \leq 2 g''(t)^{\frac{3}{2}}}
for all feasible $t$.
\end{definition}
...
\begin{itemize}
	\item the local norm $\|\cdot\|_x = \sqrt{ \langle \nabla^2A^*(x)\cdot,\, \cdot \rangle }$ and its dual local norm $\|\cdot\|_x^*= \sqrt{ \langle [\nabla^2A^*(x)]^{-1}\cdot,\, \cdot \rangle }$,
...
\subsection{Self-concordance and Convex Conjugates}
\begin{proposition}
	\label{prop:conjugateConcordance}
	Let $f:\real \rightarrow \real$ be a self-concordant function of Legendre type. Then its convex conjugate $f^*$ is also self-concordant.
\end{proposition}
\begin{proof}
When $f$ is of Legendre type then its convex conjugate $f^*(y) = \max_x \langle x, y \rangle - f(x)$ has inverse gadients
\begin{align}
    \nabla f \circ \nabla f^*  &= \Id \\
     \nabla f^* \circ \nabla f &= \Id
\end{align}
where $\Id$ is the identity function on the relevant domain. 
Deriving this equality proves that Hessians are inverse of each other 
\begin{align}
    \nabla^2 f(x) \nabla^2 f^*(x^*) = \mI
    \label{eq:conjugateHessian}
\end{align}
when evaluated at conjugate pairs of points $(x,x^*)$ -- e.g. $x^*=\nabla f(x)$ and $x = \nabla f^*(x^*)$.
These Hessians are never $0$.
In 1 dimension, this inequality is more simply written 
\aligns{f''(x) f^{* \prime \prime}(x^*) = 1}
Until now, this should be familiar to the reader.
The interesting surprise is that self-concordance appears when we derive again this equality. 
\begin{align}
    &1= f''(x) f^{* \prime \prime}(f'(x)), \forall x \\
    \implies
    &0 = f'''(x)f^{* \prime \prime}(f'(x))  + f''(x)^2 f^{* \prime \prime \prime}(f'(x))\\
    \iff
    &0 = f'''(x)f^{* \prime \prime}(x^*)  + f''(x)^2 f^{* \prime \prime \prime}(x^*)\\
    \implies
    &0 = \frac{f'''(x)}{f''(x)^{\frac{3}{2}}}  f^{* \prime \prime}(x^*)^{-\half}
    + f''(x)^{\half} \frac{f^{* \prime \prime \prime}(x^*)}{f^{* \prime \prime}(x^*)^{\frac{3}{2}}} \\
    \implies
    &0= \frac{f'''(x)}{f''(x)^{\frac{3}{2}}}  + \frac{f^{* \prime \prime \prime}(x^*)}{f^{* \prime \prime}(x^*)^{\frac{3}{2}}}
\end{align}
where to reach the last line, we used~\eqref{eq:conjugateHessian} again.
We see that for a pair of conjugate functions, the self-concordance ratios are opposite of each other, which concludes the proof.
\end{proof}
...
\rlp{Introduce SMD, in primal (minimization) and dual formulations.}
