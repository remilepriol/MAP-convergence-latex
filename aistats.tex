...
{\bf 1)} distance in parameter space. This is the focus of \emph{parameter estimation}, yielding results such as the asymptotic efficiency of the MLE via the Cramer-Rao lower-bound \citep{aitken1942estimation}.
...
One lens on this question is stochastic optimization.
...
solved by $\nat^*\in \Theta$.
Setting $f$ as in~\eqref{eq:defNLL}, the suboptimality is equal to the KL:
\alignn{
	f(\nat) - f(\nat^*) = \KL\paren{p_{\nat^*} || p_{\nat} }
