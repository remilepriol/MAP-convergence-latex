...
Now notice that~\eqref{eq:hanzely} features $\mu := \hat \mu_n = \hat\m_{n} - \lr g(\hat \nat_{n})$ stochastic and $\bar\mu := \bar\mu_n =  \hat\m_{n}   -\lr \nabla f(\hat \nat_{n})$ deterministic such that $\E_g[\mu] = \bar\mu$.
This means that
\alignn{
...
\aligns{
	\expect{\bregmanconj(\tilde \m_n , \hat\m_n} \;,
	\expect{\bregman(\hat\nat_n , \E[\hat\nat_n]} \;,
}
where expectations are on all samples $X_1, \dots, X_n$.
...
where the last two lines are due to $\E[g] = \nabla f$.
