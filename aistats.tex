...
Then the joint log-likelihood of $\cD =(X_1,\dots,X_n)$ and $\natp$ is
\begin{multline}
    -\log p(\cD|\natp)p(\natp) 
    = (n_0+n) \logpart (\natp) \\
...
\end{multline}
where $T_i=T(X_i)$.
This means the posterior given $\cD$ is part of the same family, with natural parameters $(n_0 \mu_0 + \sum_i T_i , -(n_0 + n))$.
