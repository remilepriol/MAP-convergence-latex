...
\subsection{Upper bound for the MLE of the variance}
	...
	because a Chi-squared is a special case of Gamma distributions, whose sufficient statistic is $(X, \log(X))$,
	\fdk{I don't follow why this is ``because it is a Gamma with sufficient statistics $(X,\log(X))$''?}
	\begin{align}
...
In higher dimensions, $X_i \sim \cN(0, \mu_*)$, $X_i\in\real^d$, and the mean parameter $\mu_*$ is a symmetric positive definite covariance matrix of order $d$, living in dimension $p = \frac{d(d+1)}{2}$. 
Note that we switched the role of $d$ from $\dim(T(x))=d$ to $\dim(x) =d$ and $\dim(T(x))=p$.
\fdk{This $\dim$ is not introduced? I would say that $\dim(T(X))$ is $d^2$ if $T(X) = XX^\top$. Slightly confused here.}
...
and putting it all together, we get the following closed form for the expectation of the divergence
 \begin{align}
 	...
 	\expect{\bregmanconj( \mu_*; \hat \mu_n^\text{MLE(d)})}
 	= \half \left ( \frac{2p}{n-d-1} + \psi_d\paren{\half[n]} - d \log\paren{\half[n]}  \right ).
 \end{align}
...
\begin{align}
 	...
 	\intertext{\fdk{I don't follow the second term on the next line. Is it because it is $\leq$ rather than $=$?}}
 	&\leq p \frac{n + d +1}{2 n (n-d-1)}  + \frac{(d + 1)}{2n.2 (n - d - 1)}, \\
 	&= \frac{p}{2n} + \frac{2 p(d+1)}{2 n (n-d-1)} + \frac{p/d}{2n (n - d - 1)}, \\
 	\intertext{\fdk{Why is this an inequality and }}
 	&\leq \frac{p}{2n} + \frac{p(d+2)}{n (n - d -1)}
 	\ , \forall n >d+1 \;.
\end{align}
...
\alignn{
	...
	\fdk{Fubini's theorem would be a justification here, right? Are you trying to say ``assuming we can exchange the order of integration''?}
	...
	\fdk{Was this a comment from your notes that passed its way here or do you want to go with this?}
...
\fdk{TODO: Double check the above, something doesn't sit quite right, for the following. How can we get $g'' = 0$ if $A$strictly convex?}
