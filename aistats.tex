...
For MAP, transforming~\eqref{eq:MAP_quadratic}, we get the sufficient condition \TODO{FIX THIS!} $n\geq 5d + \sqrt{5}\norm{\m^* -  \m_0} - n_0$.
...
Several pieces of work address this question under various assumptions on stochasticity \citep{hanzely2018fastest, dragomir2021fast, dorazio2021stochastic}, 
but they have so far lacked concrete examples.
In the next section, we challenge their hypothesis thanks to the MAP.

\subsection{Bounding the Randomness}

To analyze stochastic algorithms, one also needs to quantify the randomness.
While many assumptions exist for SGD \citep[\S3 for a modern review]{khaled2020better}, only a few have been adapted to SMD with relative smoothness.
We review these in the light of  the MAP and provide a summary in \cref{tbl:assumptions}.
...
However, in general, this assumption rarely holds.
...
%  \TODO{ DAMIEN short proof or formalize.}
...
This bounds the variance of the gradients at the minimum with a metric depending on the iterate $\MAPm$. This bound should hold for every iterate we may encounter on the stochastic trajectory.
...
is the analog of modern analysis of SGD~\citep[Th.3.2]{gower2019sgd},
...
Overall, this approach seems to answer~\eqref{problem}, but fails to account for the step-size $\lr_n = \inv{n_0+n}$, as \citet[Thm.1]{dorazio2021stochastic} only proves linear convergence to a variance ball of size $\frac{C}{\stgcvx}$ under constant step size.%  $\lr \leq \inv{\smooth}$. 

\subsection{Issues with current analyses}

\fdk{They do not hold for problems we care about}

\fdk{Existing techniques only deal with fixed step-sizes and show convergence to a ball}

If assumptions~\eqref{eq:hanzely} and~\eqref{eq:dragomir} do not hold for every iterate, then let us assume they hold on average. Does the proof remain valid? Yes, I think so \TODO{check this}. Does it become impossible to prove / as hard as the initial problem? Yes.
...
\section{Conclusion}
Exponential family MLE and MAP are used everywhere -- every time someone computes a mean and variance to fit some bell-shaped data.
...
The KL is a very natural measure of discrepancy between a model and the true distribution.
However, we do not have convergence rates on the expected KL, even in the well-specified setting.
Such a rate could inform us on how many samples we may need or on the influence of the prior.
There are many results close to such a rate, particularly large sample results \citep{kakade2010learning, ostrovskii2021finite}, which describe how many samples are needed to ensure a locally quadratic regime.
