...
We hope  to get a new result by combining tools from statistics and optimization. \TODO{Introduce Gaussian variance right from the start.}
...
For a full review of exponential families and their duality, see \citet[Chapter 3]{wainwright2008graphical}. \TODO{Add standard illustration} 
...
\end{align}
The MAP estimate is a random quantity. We wish to bound its deviation from the optimum $\natp^*$ or $\meanp^*$.
...
The question remains of : when is such a bound valid?  what does $\sigma^2$ depend on ? To answer these questions and more, we need to turn to examples, see  Section~\ref{sec:gaussian_variance}. 
