...
Let $f(\natp) := \expect[x]{-\log p(x|\natp)} = - \langle \mu_*, \natp\rangle + \logpartition(\natp)$. In words, $f$ is linear modification of a convex function $\logpartition$, which we can access only through noisy estimates of $\mu_*$. It turns out that the MAP estimate can also be seen as the iterates of stochastic mirror descent with mirror map $\logpartition$. First let's recall mirror descent iteration
\begin{align}
	\natp_{t+1} 
	&:= \argmin_\natp \lr \linear_f(\natp; \natp_t) + \bregman(\natp || \natp_t)  \\
	& = \nabla \conj (\nabla \logpartition(\natp_t) - \lr \nabla f(\natp_t))
\end{align}
where $\linear_f(\natp; \natp_t) = f(\natp_t) + \langle \nabla f(\natp_t), \natp - \natp_t \rangle$ is the linear approximation of $f$ in $\natp_t$ evaluated at $\natp$. Solving this problem require solving problems of the form $\argmin_\natp - \langle c, \natp \rangle + \logpartition(\natp)$, eg computing the convex conjugate of $\logpartition$. 
...
\end{align}
where $\linear_f(\natp;\natp_n, x_{t+1})$ is the stochastic linearization of $f$ at $\natp_n$ evaluated at $\natp$ with randomness coming from $x_{t+1}$. This is the formula for stochastic mirror descent (SMD) applied to $f$ with mirror map $\logpartition$ and step-size $\gamma_n = \inv{n_0 + n + 1}$.

\subsection{Relative Smoothness for Mirror Descent}
In the classic setting, SMD is studied under strong-convexity assumption on the mirror map $\logpartition$ \citep{bubeck2015convex}.
...
More precisely, he proved that when $f$ is $L$-smooth relative to $h$, then the suboptimality is upper bounded by the simple formula
\begin{align}
...
\subsubsection{Three point lemma}
These proofs rely on another basic property of Bregman divergences, familiar to information geometry folks. If $x_+$ is solution to 
\begin{align}
...
\begin{align}
	\Cov(g_t,\natp_{t+1}) 
	&= \expect{\langle g_t, \natp_{t+1} \rangle}  - \langle \E[g_t], \E[\natp_{t+1}] \rangle \geq -  \gamma_t \sigma^2, \forall t \\
	 &= \expect{\langle g_t - \nabla f(\natp_t), \natp_{t+1}  - \natp \rangle}, (\forall \natp)
\end{align}
