...
\begin{align}
	X \sim \cN(0,\sigma^2) \ ; \quad T(X)= - \half x^2 \ ; \quad  \mu = - \half \sigma^2  \\
	\natp = \lambda = \inv{\sigma^2} =  \nabla \conj (- \half \sigma^2) >0 \\
	\logpart (\lambda) = -\half \log(\lambda)  + \cst \\% \half \log \pi \\
	\conj(-\half \sigma^2) = -\half \log(\sigma^2) + \cst \\%\half (\log\frac{\pi}{2} - 1) \\
	\KL(\hat \sigma^2 ; \sigma_*^2) = \half \log \frac{\hat \sigma^2}{\sigma_*^{2}} + \frac{\sigma_*^2 - \hat \sigma^2}{2 \hat \sigma^2}
\end{align}
