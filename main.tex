...
or 
\begin{align}
	\E \bregmanconj \left (\E [T(X)] ;  \frac{n_0 \mu_0 + \smallsum_i T(x_i)}{n_0+n} \right ) = \frac{d}{2n} + O(n^{- \frac{3}{2}})
\end{align}
...
Using Jensen inequality and inserting the right quantities, we obtain the following analog of the bias-variance decomposition:
...
\end{align}
where $\hat \mu_n$ is the mean MAP estimate, $\hat \natp_n = \nabla \conj(\hat \mu_n)$ is the natural MAP estimate, 
...
The interpretation of the bias term should be clear.
The variance term is called as such because it is the covariance between $\hat \natp_n$ and $T(X)$
\begin{align}
	\Cov \bigg(T(X_0), \nabla \conj \big(\frac{\smallsum_i T(X_i) +n_0 \mu_0}{n+n_0} \big)\bigg	) \; .
\end{align}
The mixed bias variance term is might be negative. 
\TODO{add figure.}
Note that unless $\conj$ is $\ell^2$, or degenerate 0 variance cases, $\theta_n \neq \expect{\hat \theta_n}$.
In fact, $\theta_n - \expect{\hat \theta_n} = \nabla \conj(\E[\hat \mu_n]) - \E[\nabla \conj(\hat \mu_n)]$ is sort of a commuting bracket. 
...
\end{align}
Using a linear approximation of the mirror map, we also get a $O(n^{-1})$ rate for the variance term
\begin{align}
	&\nabla \conj(\mu)  
	= \nabla \conj(\mu^*) + \nabla^2\conj(\mu^*)(\mu - \mu^*) + O(\norm{\mu - \mu^*}^2) \\
	\implies &\hat \natp_n 
	= \natp^* + \Sigma^{-1} (\hat \mu_n - \mu^*)  O(\norm{\hat \mu_n - \mu^*}^2) \\
	\implies &\Cov(\hat \natp_n ; T(X) ) 
	= \inv{n+n_0} \E[ \norm{T(X) - \mu^*}^2_{\Sigma^{-1}}]   + O(n^{-2}) \\
	\implies &\Cov(\hat \natp_n ; T(X) ) 
	 = \frac{d}{n+n_0} + O(n^{-2}) \; .
\end{align}
where we lost the factor $2$ compared to the straightforward asymptotic analysis, probably because of the Jensen upper inequality.

Finally, we can use this same linear approximation to bound the mixed bias-variance term
