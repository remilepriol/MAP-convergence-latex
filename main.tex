...
\begin{align}
	\quad T(X)= x^2 \ ; \quad  \mu = \sigma^2 \ ; \quad   \natp = -\inv{2 \sigma^2} = -\half \lambda \; .
\end{align}
Nw we can match the log-likelihood with the exponential family template to get the log-partition function.
\begin{align}
	-\log p(x) = \frac{x^2}{2 \sigma^2} - \half \log(2\inv{2\sigma^2}) + \half \log(2 \pi) 
	= -x^2 \lambda + \logpart(\lambda) \\
	\implies \logpart (\lambda) = -\half \log(-\lambda)  + \half \log(\pi)
\end{align}
We can then use the formula $\conj(\mu) = \mu \natp - \logpart(\natp)$ to get the entropy
\begin{align}
	\conj(\sigma^2) = -\half \log(\sigma^2) + \half \log\frac{\pi}{2} - \half \\
\end{align}
Remark that both the entropy and the log-partition have the same shape $z\mapsto -\inv{2z}$. Which yields the same Bregman divergence
\begin{align}
