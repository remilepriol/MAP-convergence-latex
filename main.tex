...
\begin{align}
	...
	\leq \lr_n M \; .
\end{align}
on the sufficient statistic and $\conj$. Unfortunately this bound is not trivially satisfied, even for simple exponential family members, such as centered gaussians $\cN(0,\sigma^2)$ with natural parameter $\lambda > 0$ the precision and sufficient statistic $T(X)= - \half x^2$. The mean parameter is $- \half \sigma^2$ and  the conjugate map is $\nabla \conj (- \half \sigma^2) = \inv{\sigma^2} = \lambda$. 
\begin{align}
	\logpart (\lambda) = -\half \log(\lambda)  + \half \log \pi \\
	\conj(-\half \sigma^2) = -\half \log(\sigma^2) + \half (\log\frac{\pi}{2} - 1) \\
	\KL(\hat \sigma ; \sigma_*) = \half \log \frac{\hat \sigma^2}{\sigma_*^{2}} + \frac{\sigma_*^2 - \hat \sigma^2}{2 \sigma^2}
\end{align}
...
\end{align}
using a first order approximation of the inverse , eg a first order approximation of $\nabla \conj$. This can explode for small values of $\sigma_0$. In fact it does as illustrated experimentally.
