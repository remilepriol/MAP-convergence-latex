...
\begin{example}
	\paragraph{MLE exact computation}
...
\begin{example}
	\paragraph{MAP upper bound.}
	Wolfram finds no closed form for $\expect{\log\frac{\MAPm}{\mu^*}}$ but it does for $\expect{\frac{\mu^*}{\MAPm}} = \expect{\frac{\MAPt}{\natp^*}}$. 
	To start, let us plug in the definition of $\MAPm$
	\begin{align}
		\E[\MAPt] &= -\frac{n+n_0}{2}\expect{\inv{n_0 \mu_0 + \sum_i X_i^2}}  \\
		&= -\frac{n+n_0}{2 \mu^*}\expect{\inv{n_0 \frac{\mu_0}{\mu^*}+ \sum_i \frac{X_i^2}{ \mu^*} }} \\
		&= -\frac{n+n_0}{2 \mu^*}\expect{\inv{a + \chi^2(n)}}
	\end{align}
	where $\sum_i \frac{X_i^2}{ \mu^*}$ is a chi-square random variable of degree $n$ and 
	\begin{align}
			a=n_0 \frac{\mu_0}{\mu^*} \; .
	\end{align}
	This expectations is not simple to solve, but using wolfram, we get  the formula :
	\begin{align}
		\expect{\inv{a + \chi^2(n)}}  = \half e^{\frac{a}{2}} E_{\half[n]}(\half[a])
		\label{eq:wolfram_input}
	\end{align}
	...
	When $n=2$, we have the simple exponential integral function which \href{https://dlmf.nist.gov/6.8#E1}{verifies the bound}
	\begin{align}
		\half \log(1 + \frac{2}{x}) 
		\leq e^x E_1(x) 
		\leq \log(1+\inv{x}) \leq \inv{x} \; .
	\end{align}
	For larger $n$, we have \href{https://dlmf.nist.gov/8.19#E21}{the general bound}
	\begin{align}
		\inv{x+n}
		\leq e^x E_k(x) 
		\leq \inv{x + k - 1}
	...
	\begin{align}
		\frac{n +n_0}{2 \mu^* (a + n)}	
		&\leq - \E[\MAPt]
		\leq \frac{n +n_0}{2 \mu^* (a + n - 2)}	\\
		\iff \inv{2 \mu_n}
		&\leq - \E[\MAPt]
		\leq \inv{2 \mu_n - \frac{4 \mu^*}{n + n_0}}	\\
		\iff \frac{\mu^*}{\mu_n}
		&\leq \expect{\frac{\mu^*}{\MAPm}} 
		\leq \frac{\mu^*}{\mu_n - \frac{4 \mu^*}{n + n_0}}  
		\label{eq:theta_expectation_bound}
	\end{align}
	because $\frac{\mu^* (a + n)}{n +n_0} = \mu_n$.
Now using the bound \eqref{eq:log_bound} on the logarithm, we get
\begin{align}
	2\expect{\bregmanconj(\mu^*; \MAPm)} 
	&\leq \expect{\frac{\mu^*}{\MAPm}}  + \expect{\frac{\MAPm}{\mu^*}} - 1 \\
	&\leq \frac{\mu^*}{\mu_n - \frac{4 \mu^*}{n + n_0}}   + \frac{\mu_n}{\mu^*} - 1 \\
...
This inequality highlights a clear variance-bias decomposition, to compare with the MLE. Unfortunately it is not tight asymptotically as the inequality \eqref{eq:log_bound} is far from tight. We are basically losing a factor 2 compared to $1/2n$. 

\paragraph{MLE Loose Upper Bound.}
If we apply \eqref{eq:log_bound} to the MLE instead, we get the upper bound
\begin{align}
	...
	= \frac{1}{n-2}
\end{align}
which is exactly $1/2n$ larger than \eqref{eq:tight_bounds} which we got thanks to an exact computation of the expectation of the log term, and a tight upper bound on the digamma function.
\end{example}

	\begin{figure}[ht]
    	\centering
	    \includegraphics[width=.7\textwidth]{figs/wolfram.png}
	    \caption{Output of Wolfram when queried for the integral \eqref{eq:wolfram_input}}
    	\label{fig:wolfram}
	\end{figure}
