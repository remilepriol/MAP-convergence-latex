...
\label{eq:hypvariance}
	...
	\leq \lr \sigma^2 \; .
\end{align}
When we plug this bound into the expected suboptimality formula~\eqref{eq:bregmanconj} with the maximum likelihood estimate $\hat \mu =  \inv{n}\sum_i T(X_i)$, assuming all $T(x)$ belong to the marginal poytope, we get
\begin{multline}
