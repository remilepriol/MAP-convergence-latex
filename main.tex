...
The relationship between Bregman divergences and Fenchel conjugacy is well explained in Wainwright's book, and the article geometry of exponential family. \RLP{add citations} 
The question is: how does this quantity behave when $\natp$ is the maximum-likelihood or the MAP estimate ? Can we get bounds -- in expectation or high-probability ?
\begin{align}
	\label{eq:bregmanMLE}
...
Let us make these statements more precise. 
If $\Var T(X) = \sigma^2$, then 
\begin{align}
	\E \norm{\mu^* -  \inv{n}  \smallsum_i T(x_i)}^2 = \frac{\sigma^2}{n} 
\end{align}
eg the variance of the mean is $n$ times smaller than the variance of the samples. We can extend this result to a Manahalobis distance with any matrix $\mM$
\begin{align}
	\E \norm{\mu^* -  \inv{n}  \smallsum_i T(x_i)}_\mM^2 = \inv{n} \Tr(\mM \mSigma)
\end{align}
where $\mSigma$ is the covariance of $T(X)$. This  bound is equal to $\nicefrac{\sigma^2}{n}$, the variance divided by the number of samples, when the metric is the identity $\mM=\mI$. 
Adding a reference mean $\mu_0$ to get the MAP,
\begin{align}
...
\begin{align}
    \hat \natp_{n+1} 
    &= \argmin_\natp -\log p(x_{n+1}|\natp) + (n_0 + n) \bregman(\natp || \hat \natp_n) \\
    &= \argmin_\natp f(\natp; x_{n+1}) +  \inv{\gamma_n}\bregman(\natp || \hat \natp_n) \; .
\end{align}
...
However a recent and fast-expanding body of work is concerned with a new assumption: relative smoothness and relative strong-convexity. $f$ is $\mu$ strongly-convex and $L$-smooth relative to $h$ if
\begin{align}
	& f-\mu h \ \text{ convex and } Lh - f \ \text{convex} \\
	\iff & f(x) + \langle \nabla f(x) , y-x \rangle + \mu \cB_h(y||x) \leq f(y) \\
	&\text{and} \quad f(y) \leq 	f(x) + \langle \nabla f(x) , y-x \rangle + L\cB_h(y||x), \forall x,y \\
	\iff & \mu\cB_h(y||x) \leq \cB_f(y||x) \leq L \cB_h(y||x), \forall x,y \\
	\iff & \mu \nabla^2 h(x) \leq \nabla^2 f(x) \leq L \nabla^2 h(x),  \forall x
\end{align} 
where the last equivalence holds only when $f$ and $h$ are twice differentiable. 
This sweet generalization of smoothness and strong-convexity transfers the Loewner partial order on symmetric matrices to functions, via the Hessian. 
...
This is very interesting for us, but the variance hyper-parameter is oddly defined. Let $g_t$ be the random gradient at step $t$  (coming from data point $x_t$), and $\natp_{t+1}$ the next iterate. Then the variance bound $\sigma^2$ is a lower bound on the covariance between $g_t$ and $\natp_{t+1} = \nabla \logpart^*(\nabla \logpart(\natp_t) - \lr_t g_t )$. 
\begin{align}
	\Cov(g_t,\natp_{t+1}) 
	&= \expect{\langle g_t, \natp_{t+1} \rangle}  - \langle \E[g_t], \E[\natp_{t+1}] \rangle \geq -  \lr_t \sigma^2, \forall t \\
	 &= \expect{\langle g_t - \nabla f(\natp_t), \natp_{t+1}  - \natp \rangle}, (\forall \natp)
\end{align}
where $\gamma_t$ is the step-size and expectations are conditional on $\natp_t$. In words, $g_t$ and $\natp_{t+1}$ should not be too negatively correlated. In our case, 
\begin{align}
	g_t - \nabla f(\natp_t) = \E[T(X)] - T(X_{t+1})	
\end{align}
...
\end{align}
In other words, we need for all $\lr\in(0,1)$, and all $\mu$ in the interior of the marginal polytope,
\begin{align}
\label{eq:hypvariance}
	\Cov_X \bigg(T(X), \nabla \conj (\lr T(X) + (1-\lr)\mu)\bigg	) 
	\leq \lr \sigma^2 \; .
\end{align}
When we plug this bound into the expected suboptimality formula~\eqref{eq:bregmanMLE} with the maximum likelihood estimate $\hat \mu =  \inv{n}\sum_i T(X_i)$, assuming all $T(x)$ belong to the marginal poytope, we get
\begin{multline}
...
\end{multline}
where we used the decomposition $\hat \mu = \inv{n}T(X_i) + (1-\inv{n}) \inv{n-1} \sum_{j\neq i}T(X_j)$ to apply~\eqref{eq:hypvariance}. In words, this variance assumption on $T(X)$ and $\conj$ immediately gives us a bound on the suboptimality. The question remains of : when is such a bound valid? For which values of $\sigma^2$. To answer such questions, we need to turn to examples. 
