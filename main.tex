...
\begin{lemma}[Expected MAP natural parameter]
	...
	\end{align}
\end{lemma}
Using this lemma, along with~\eqref{eq:log_bound}, we get the following result.
...
\begin{equation}
	...
	\leq \begin{cases}
		...
		\frac{1+n_0}{4} e^{\half[a]} \sqrt{\frac{2 \pi}{a}} \mathrm{erfc}(\sqrt{\half[a]})  - \half  + \frac{a - n_0}{2(1+n_0)} \ \text{if}\ n=1,\\
...
\begin{proof}
	...
	\begin{align}
			a=n_0 \frac{\mu_0}{\mu^*} \; .
	\end{align}
	This expectations is not simple to solve, but using the trick
	\begin{align}
		&\int_1^\infty e^{-\frac{zt}{2}} dt = \frac{2}{z} e^{-\half[z]} 
		\ \implies \ \inv{z} = \half e^{\half[z]} \int_1^\infty e^{-\frac{zt}{2}} dt
		\\
		\implies 
		&\inv{a+x} = \half e^{\frac{a+x}{2}} \int_1^\infty e^{- \frac{(a+x)t}{2}}dt \; ,
	\end{align} 
	we get  the formula :
	\begin{align}
		(n_0 +n) \expect{\inv{a + \chi^2(n)}}  = \half[n_0+n] e^{\frac{a}{2}} E_{\half[n]}(\half[a])
	...
	or 
	\begin{align}
		E_{\half[1]}(\half[a]) = \sqrt{\frac{a}{2\pi}}
	\end{align}
	...
	\begin{align}
		...
		\iff \frac{\mu^*}{\mu_n}
		&\leq \expect{\frac{\mu^*}{\MAPm}} 
		\leq \frac{\mu^*}{\mu_n - \frac{2 \mu^*}{n + n_0}}  
...
This inequality highlights a clear variance-bias decomposition, to compare with the MLE. Unfortunately it is not tight asymptotically as the inequality \eqref{eq:log_bound} is not quadratically tight around $1$. We are basically losing a factor 2 compared to $1/2n$. 
\end{proof}
...
\subsection{MLE Tight Upper Bound}
	...
	Its expected suboptimality is infinite when $n\leq 2$ and 
	\begin{align}
		 \expect{\bregmanconj( \mu_*; \mu_n) }
			\leq \inv{2n} +\frac{2}{n(n-2)}
	\end{align}
	otherwise. Note that this upper bound is asymptotically tight.
	\end{theorem}
...
\begin{align}
	...
	= \frac{1}{n-2}
\end{align}
which is exactly  an additive term $1/2n$ larger than \eqref{eq:tight_bounds}. This highlights the factor $2$ difference between the log-bound and an exact computation of the expectation of the log term, and a tight upper bound on the digamma function.
