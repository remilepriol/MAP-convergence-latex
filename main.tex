...
\end{align}
where $\mSigma$ is the covariance of $T(X)$. This is equal to $\nicefrac{\sigma^2}{n}$, the variance divided by the number of samples, when the metric is the identity $\mM=\mI$. It is worth $\nicefrac{d}{n}$ when $\mM = \mSigma^{-1}$. \RLP{Maybe that is Cramer-Rao, but check again.} And for the MAP we  get 

\begin{align}
	\E \norm{\mu^* -  \frac{n_0 \mu_0 + \smallsum_i T(x_i)}{n_0+n} }^2_{\mSigma^{-1}}
	&= \frac{n}{(n+n_0)^2} \Tr(\mSigma^{-1}\mSigma) +  \frac{n_0^2}{(n+n_0)^2} \norm{\mu^* -  \mu_0}^2_{\mSigma^{-1}} \\
