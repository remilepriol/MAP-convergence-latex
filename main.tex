...
Stochastic Mirror Descent (SMD) was studied under relative smoothness assumptions in 2 papers : \citet{hanzely2018fastest} and \citet{dragomir2021fast}.
...
\begin{align}
	\lr_t \sigma^2 \geq 
	&\Cov(- g_t,\natp_{t+1} - \natp_t | X_{1 \dots t})  \label{eq:hanzely_hypothesis} \\
	&= \expect{\langle - g_t, \natp_{t+1} \rangle}  - \langle \E[-g_t], \E[\natp_{t+1}] \rangle   \\
	 &= \expect{\langle \nabla f(\natp_t) - g_t , \natp_{t+1}  \rangle}\\
	 &= \expect[X_t]{\bregman(\natp_{t+1} ; \bar \natp_{t+1} ) + \bregman(\bar \natp_{t+1} ; \natp_{t+1} ) }
\end{align}
where $\gamma_t$ is the step-size, expectations are conditional on the past, and $\bar \natp_{t+1}$ is the theoretical output of a deterministic Bregman gradient step. Remark that when we plug in $\conj = \|.\|^2$, we recover the gradient variance typical of SGD. 
