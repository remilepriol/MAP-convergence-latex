...
\begin{lemma}[Expected MAP natural parameter]
	...
	\end{align}
\end{lemma}
Not that the lower bound is a simple consequence of the convexity of the inverse function. Using this lemma, along with~\eqref{eq:log_bound}, we get the following result.
...
\begin{proof}
	...
	Now with the change of variable $x = 2\frac{y}{t}$, eg $dx = 2\frac{dy}{t}$, the inner integral becomes \TODO{finish}.
	we get  the formula :
	\begin{align}
	...
	\end{align}
	When $n=1$, the integral can be written with the complementary error function
	\begin{align}
		E_{\half[1]}(\half[a]) = \sqrt{\frac{2\pi}{a}} \mathrm{erfc}(\sqrt{\half[a]}) \; .
	\end{align}
	...
	\end{align}
	where we used $\frac{a + n}{n +n_0} = \mu_n$.
\end{proof}
