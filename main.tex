...
Resuming from the Stochastic Bregman Proximal Point formula~\eqref{eq:SBPP} and manipulating the Bregman yields
\begin{align}
    \hat \natp_{n+1} 
    &= \argmin_\natp 
    - \langle T(x_{t+1}), \natp \rangle  + \logpart(\natp) + (n_0 + n) \bregman(\natp ; \natp_n) \\
...
\TODO{Sanity check literature search with Bach's references.}
