...
\end{enumerate}
Regarding this second point, it might be possible to do something comparable to \citet{gower2019sgd} who get a $O(\inv{t})$ rate with a $O(\inv{t})$ step-size. Take $m$ and $L$ as the (relative) strong-convexity and smoothness constants of the problem.Their descent lemma looks like
\begin{align}
...
\begin{align}
	\lr_t := \inv{m} \left ( 1 - \left (\frac{k}{k+1} \right)^\alpha \right ) \; .
\end{align}
